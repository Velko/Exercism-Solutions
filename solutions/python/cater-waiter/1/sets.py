...
def check_drinks(drink_name, drink_ingredients):
    ...
    if ALCOHOLS.isdisjoint(drink_ingredients):
        name_postfix = " Mocktail"
    else:
        name_postfix = " Cocktail"

    return drink_name + name_postfix
...
def categorize_dish(dish_name, dish_ingredients):
    ...
    the dish belongs to).`<CATEGORY>` can be any one of  (VEGAN, VEGETARIAN, PALEO, KETO, or
...
def tag_special_ingredients(dish):
    ...
    Return the dish name followed by the `set` of ingredients that require a special note on
    the dish description. For the purposes of this exercise, all allergens or special ingredients
    that need to be tracked are in the SPECIAL_INGREDIENTS constant imported from
    `sets_categories_data.py`.
    """
